...
(function($) {
    ...
    function init(type, options) {
        ...
        var data = app.mobileMode && app.mobileScriptVersion === 2 ? d : JSON.stringify(d);
...
app.charts.gauge.draw = function(input, settings, refreshWithData, titlebar) {
    ...
    function createArcGaugeMultiSectionV2(n, divdata, divindex, chartHeight, chartWidth) {
        var showLabel = true;
        var w = Math.min(chartHeight, chartWidth);
        var scale = w / 200;
        var valueSize = showLabel ? 16 : 20;
        var outerRadius = showLabel ? 80 : 100;
        var innerRadius = outerRadius - 30;
        var smallRadius = outerRadius - 55;
        var divs = d3.select(n).style("transform", "scale(" + scale + ")").selectAll("nothings").data(function(d) {
            return d.kpis;
        ...
        divs.append("img").attr("src", app.urlPrefix + "api/upload/getPic/231").styles({
            margin: "4px",
            position: "absolute",
            width: outerRadius * 2 + 12 + "px",
            left: "0",
            top: "0"
        });
        var svgs = divs.append("div").style("margin-bottom", "0").append("svg").attr("width", outerRadius * 2).attr("height", outerRadius * 2).style("margin", "10px").append("g").attr("transform", "translate(" + outerRadius + "," + outerRadius + ")");
        if (showLabel) {
            divs.append("div").text(function(d, i) {
                var kl = kpisLabels[i];
                return persian(divdata.label + (kl ? " - " + kl : ""), showPersian);
        ...
        function drowArcAll(outerRadius, innerRadius) {
            var arc = d3.arc().innerRadius(outerRadius).outerRadius(innerRadius).startAngle(function(d) {
                return d.start;
            }).endAngle(function(d) {
                return d.end;
            });
            function drowArc(startIndex, endIndex, redColor) {
                svgs.append("path").attr("d", function(d, i) {
                    var map = d.map(function(dd, ii) {
                        return xarray[i](dd);
                    });
                ...
                }).attr("fill", redColor).attr("transform", "translate(" + 0 + "," + 0 + ")");
            }
            drowArc(3, 4, redColor);
            drowArc(4, 5, yellowColor);
            drowArc(5, 6, greenColor);
        }
        drowArcAll(outerRadius, innerRadius);
        drowArcAll(smallRadius + 10, smallRadius);
        function drowValue(callBack) {
            svgs.append("text").attr("text-anchor", "middle").styles({
                "font-size": "16px",
                "font-family": "IRANSans",
                "font-weight": "bold"
            }).attr("dy", "-0.3em").text(function(d) {
                return persian(d3.format(font.formatString)(callBack(d)), showPersian);
            }).attr("transform", function(d, i) {
                return "rotate(" + xarray[i](callBack(d)) * (180 / Math.PI) + ")" + "translate(" + 0 + "," + (-outerRadius + 25) + ")";
            });
        }
        for (var i = 0; i <= 10; i++) {
            drowValue(function(d) {
                return d[3] + (d[6] - d[3]) / 10 * i;
            });
        }
        var needle = svgs.append("g");
        needle.append("rect").attr("width", "3").attr("transform", "translate(" + (0 - 1.5) + "," + 0 + ")").attr("height", outerRadius - 25).attr("fill", "#333");
        needle.attr("transform", "rotate(" + (startAngle * (180 / Math.PI) + 180) + ")");
        needle.transition().duration(settings.animationDuration).attrTween("transform", function(d, i) {
            var x0 = xarray[i](d[0] < d[3] ? d[3] : d[0] < d[6] ? d[0] : d[6]);
            var zero = startAngle * (180 / Math.PI) + 180;
            var final = zero;
        ...
        svgs.append("circle").attr("r", smallRadius).styles({
            "stroke-width": "2px",
            stroke: "#333",
            fill: "#fff"
        });
        svgs.append("circle").attr("r", outerRadius).styles({
            "stroke-width": "3px",
            stroke: "#121212",
            fill: "none"
        });
        var gaugeVal = svgs.append("text").attr("class", "gauge-value").attr("text-anchor", "middle").style("fill", font.color).style("font-family", font.fontName).attr("dy", "0.3em").style("font-size", valueSize + "px").style("font-weight", font.bold ? "bold" : "normal").style("font-style", font.italic ? "italic" : "normal").text(function(d) {
            return persian(d3.format(font.formatString)(d[0]), showPersian);
        }).attr("transform", "translate(" + 0 + "," + 0 + ")").transition().duration(settings.animationDuration).tween("text", function(d) {
            var i = myInterpolate(d[3], d[0]);
            var node = this;
            return function(t) {
                node.textContent = i(t);
...
String.prototype.entitize = function(isText) {
    var ret = this;
    var rr = this;
    ret = ret.replace(/&/g, "&amp;");
    ret = ret.replace(/>/g, "&gt;");
...
app.charts.userControl.draw = function(input, settings, refreshWithData, titlebar) {
    ...
    function drawCombo(isMulti) {
        ...
        $(selector + " .ui.dropdown").dropdown({
            ...
            apiSettings: {
                ...
                onResponse: function(res) {
                    console.log("### onResponse = " + JSON.stringify(res));
                    res.fields = res.fields.map(function(d) {
                        return {
                            Key: d.Key.entitize(true),
                            Value: d.Value.entitize()
                        };
                    });
    ...
    function getFilters() {
        ...
        var userVariable = [];
        if (!app.mobileMode) {
            userVariable = JSON.parse(localStorage.getItem("userVariable")) || [];
        }
        return {
...
ngApp.controller("dashboardPageBodyCtrl", [ "$scope", "$http", "$sce", "$routeParams", "navbar", "$rootScope", "$timeout", "drawer", "$q", "$rootScope", "dashboardContainerService", function($scope, $http, $sce, $routeParams, navbar, $rootScope, $timeout, drawer, $q, $rootScope, dashboardContainerService) {
    ...
    function renderBody() {
        ...
        $http({
            ...
            if ($scope.paramkey && !app.mobileMode) {
                var allParams = JSON.parse(localStorage.getItem("filter-params") || "[]");
...
function fixLayout(forceRefresh) {
    ...
    var onFinish = function() {
        if (typeof Storage !== "undefined" && !app.mobileMode) {
