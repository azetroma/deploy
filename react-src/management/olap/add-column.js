...
export default class AddColumn extends React.Component {
    constructor(p) {
        ...
        this.map = p.data.Hierarchies.map(function (item) {
            return {
                label: item.Name,
                apply: item.UniqueName,
                type: 'variable'
            };
        });
    }
    ...
    addColumn(isMeasure) {
        ...
        let name = isMeasure ? 'شاخص جدید' : 'ستون جدید';
        let i = 1;
        while (_.indexOf(names, name) !== -1) {
            name = (isMeasure ? 'شاخص جدید' : 'ستون جدید') + ' ' + i++;
        }

        this.setFormula('');
        this.setState({ open: true, name: name, id: 0, formula: '', isMeasure: isMeasure });
    }
    ...
    delete() {
        ...
        Axios
            .post(app.urlPrefix + 'api/olap/deleteColumn/' + this.props.model.CalcId)
            .then(res => {
                        ...
                        <Modal.Content scrolling>
                            <Modal.Description>
                                <div className="ui form">
                                    <div className={"field" + (this.props.model && this.props.data.ModelType == 1 ? ' xdisabled' : '')}>
                                    ...
                                    <div className="field" >
                                        ...
                                        <div className="ltr" style={{ textAlign: 'left', border: '1px solid #efefef', borderRadius: '4px' }}>
                                            <CodeMirror
                                                extensions={[StreamLanguage.define(mySQL), autocompletion({ override: [this.myCompletions] })]}
                                                ref="CodeMirror" className="ltr" value={this.state.formula || ''} onChange={(d) => this.updateCode(d)} options={options} />
                                        </div>
                                    </div>
                                    {this.showError()}
                                </div>
        ...
        return (
            <div>
                <Dropdown icon='ellipsis vertical small link gray'>
                    <Dropdown.Menu>
                        {this.props.model && <Dropdown.Item text={this.props.model ? 'ویرایش' : 'شاخص جدید'} onClick={() => this.addColumn(true)} />}
