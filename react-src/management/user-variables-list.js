...
export default class UserVariablesList extends React.Component {
    ...
    componentDidMount() {
        get(app.urlPrefix + 'api/userVariables/get')
            .then(data => {
    ...
    delete(item) {
        ...
        fetch(app.urlPrefix + 'api/LoginPageLogo/delete/' + item.id).then(() => {
            let list = [...this.state.data];
        ...
        return (
            <div>
                ...
                <Menu pointing secondary>
                    <Menu.Item active name="لیست متغیرهای کاربری" />
                </Menu>
                
                <table className="ui table">
                    <thead><tr><th style={{ width: '50px' }}>#</th><th>نام متغیر</th></tr></thead>
                    <tbody>
                        {this.state.data && this.state.data.map((item, i) => {
                            return (
                                <tr key={i}>
                                    <td>{persian(i + 1, true)}</td>
                                    <td className="pointer" onClick={() => this.edit(item)}> <a href={app.hashUrlPrefix + '#/userVariables/' + item}>{item}</a></td>
                                </tr>
                            );
                        })}
                    </tbody>
